...
def cluster_abs(abstracts, num_clusters, pmids):  # Tf-idf Vectorization and Kmeans- Clustering
    ...
    joblib.dump(km, './pubmed_cluster.pkl')  # Save the model using joblib because centroids change if we re-run.
...
def main():
    f = pd.read_csv('./pmids_gold_set_labeled.txt', sep='\t', header=None)  # read input file
    pmids = list(f[0])

    with open('./abstractslist.txt', 'r', encoding='utf-8') as fin:   # read the retrieved abstracts from file
        abs_list = fin.read().split('\n')

    num_clusters = 6    # select number of clusters
